...
export default (props) => {
  ...
  api.modifyConfig(memo => {
    ...
    let routers = []; // 如果传入了 layout 只展示当前的

    if (componentPath) {
      routers.push(_objectSpread(_objectSpread({}, options.menu), {}, {
        path,
        component: (0, _path().join)('../', process.argv.slice(2)[1], './src/index'),
        exact: false
      })); // 否则展示所有的
    } else {
      const components = (0, _fs().readdirSync)(cwd).filter(componentsPath => {
        if ((0, _fs().existsSync)((0, _path().join)(componentsPath, 'package.json'))) {
          return true;
        }
      ...
      routers = components.map(pagePath => {
        return _objectSpread(_objectSpread({}, options.menu), {}, {
          path: pagePath.toLocaleLowerCase(),
          name: pagePath.toLocaleLowerCase(),
          component: (0, _path().join)('../', pagePath, './src/index'),
          exact: false
        });
      });
    }
