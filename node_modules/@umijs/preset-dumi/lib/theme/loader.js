...
function getThemeResolvePaths(sourcePath) {
  ...
  let compiledModulePaths; // search compiled module in es/lib instead of src firstly, for compatible with mfsu

  if (srcRegExp.test(sourcePath)) {
    try {
      compiledModulePaths = getThemeResolvePaths(sourcePath.replace(srcRegExp, '/es/$1')) ||
      /* istanbul ignore next */
      getThemeResolvePaths(sourcePath.replace(srcRegExp, '/lib/$1'));
    } catch (err) {
      /* nothing */
    }
  }
...
var _default = /*#__PURE__*/_asyncToGenerator(function* () {
  if (!cache || process.env.NODE_ENV === 'test') {
    ...
    (0, _utils().winPath)(_path().default.dirname(getThemeResolvePaths(theme).resolved)); // local theme has no src directory but theme package has
    ...
    const components = _fs().default.existsSync(builtinPath) ? _fs().default.readdirSync(builtinPath).filter(file => {
      const absPath = _path().default.join(builtinPath, file); // filter .js/.ts/.jsx/.tsx, and exclude .d.ts


      const isFileComponent = /((?<!\.d)\.ts|\.(jsx?|tsx))$/.test(file); // filter Foo/index.(tsx|jsx|js|ts)
